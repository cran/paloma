...
assign("Gplot.graph",
       list( pie.gr=NULL, default.gr=NULL, reso=2000,
            ...
            col= light.palette(10,black=FALSE),
            dust.bin = NULL ),
           envir=Gplot.env ) 
...
Gplot.default <- function ( X, ... ) {
  ...
  if ( nb.lev > length( class.col )) {
    ...
    class.col <- light.palette( nb.lev, black=FALSE ) 
  }
  ...
  if ( is.vector( node.pie.coef ) ){
    if( length( node.pie.coef ) == 1) {
      if( length(save.class.col) < NNodes ) {
        ...
        save.class.col <- light.palette( NNodes, black=FALSE ) 
    ...
    if( length(save.class.col) < dim(node.pie.coef)[2] ) {
        ...
        #                                         black=FALSE ) )
        save.class.col <- light.palette( dim(node.pie.coef)[2],
                                                 black=FALSE )
